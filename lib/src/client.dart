...
class HttpPlusClient extends BaseClient {
  ...
  Future<StreamedResponse> _sendHttp2(
    ...
    // TODO: use stream.peerPushes to listen and process

    // stream request data to stream - and then close outgoing sink
    await requestStream.forEach((bytes) => stream.sendData(bytes));
  ...
  Future<StreamedResponse> _makeResponse(BaseRequest request,
    ...
    void complete() {
      ...
      if (statusCode == null) {
        return responseCompleter.completeError(StateError(
      ...
      if (request.followRedirects &&
        ...
        if (redirects.length >= request.maxRedirects) {
          return responseCompleter.completeError(RedirectException(
    ...
    stream.incomingMessages.listen(
      (message) {
        if (message is HeadersStreamMessage) {
          ...
          responseCompleter.completeError(ArgumentError.value(message,
              'message', 'must be HeadersStreamMessage or DataStreamMessage'));
        }
      },
      cancelOnError: true,
      onDone: () {
